...
## preprocess
# Load csv file
df_ = pd.read_csv('./total/salary_clean_tokenize_with_marks.csv', encoding='utf-8')
df_.drop('Unnamed: 0', axis=1,inplace=True)
df_open = df_[~df_['month_salary'].isna()].reset_index(drop='index')
_df = df_open[['aID','company', 'vacancies', 'post_time', 'whole_art', 'sentence','word']]
_df = _df.rename(columns = {'whole_art':'result','company':'company_name'})
_df['word'] = _df['word'].apply(lambda x: eval(x))
_df = _df[~_df['vacancies'].str.contains('客服|專員')].reset_index(drop='index')
print("data shape: ",_df.shape)
# # 篩選公司
...
## DTM
data_list = [{k: v for k, v in d.items() if k not in ['word']} for d in dict_] # 去除不需要的斷詞欄位，剩下的資料為畫趨勢圖的dtm
dtm_df = pd.DataFrame(data_list) # 得到for 趨勢圖的 DTM DataFrame
...
## save
dtm_df.to_csv('./test/salary/salary_word_dict_dtm_TWM.csv', encoding='utf-8', index=False)
corr_df.to_csv('./test/salary/salary_word_dict_corre_TWM.csv', encoding='utf-8-sig')
ocurr_df.to_csv('./test/salary/salary_word_dict_CoOcurr_TWM.csv', encoding='utf-8-sig')