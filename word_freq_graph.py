...
if start_date and end_date and aspect_option:
    ...
    if (df_select.shape[0] == 0):
        ...
        # 生成日期範圍並轉換為所需的字符串格式
        #
        date_range = pd.date_range(start="{}-{}".format(start_date.year, start_date.month), end="{}-{}".format(end_date.year+1 if end_date.month == 12 else end_date.year , 1 if end_date.month == 12 else end_date.month), freq='M', inclusive="both").strftime('%Y-%m').tolist()
